...
def user_input():
    user_cross = []
    while True:
        user_cross = list(map(int, list(input("Ваш ход, поставьте крестик по принципу: строка колонка): ").split())))
        if len(user_cross) == 2 and all(list(map(lambda x: x in range(N), user_cross))):
            ...
            if empty_cell(row, col):
                print("Вы выбрали: ", user_cross)
                return user_cross
            else:
                print("Увы, ячейка занята, попробуйте другую")
        else:
            print("Увы, выходить за пределы поля нельзя")
...
while count_empty_cells() > 0:
    ...
    if check_win('O'):
        fprint(field)
        print("Что ж, повезёт в следующей партии!")
        break
